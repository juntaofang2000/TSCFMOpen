...
def _predict_direct(
    ...
    *,
    ...
    """Directly predict labels for X_query using (support + query) ICL episodes."""

    device = next(model.parameters()).device
    X_support_t = torch.from_numpy(np.asarray(X_support, dtype=np.float32)).to(device)
    X_query_t = torch.from_numpy(np.asarray(X_query, dtype=np.float32)).to(device)
    ...
    if X_query_t.dim() != X_support_t.dim():
        raise ValueError(f"Support/query dim mismatch: support={tuple(X_support_t.shape)}, query={tuple(X_query_t.shape)}")
