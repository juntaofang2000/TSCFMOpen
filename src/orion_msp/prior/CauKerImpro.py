...
def sample_from_gp_prior_efficient_gpu(
    ...
    if cp is None:
        ...
        return rng.multivariate_normal(mean=mean_vec, cov=cov_cpu, method=method)
...
def random_activation(x: np.ndarray, func_type: str = "linear") -> np.ndarray:
    ...
    if func_type == "sigmoid":
        return 1.0 / (1.0 + np.exp(-x))   #可能有溢出 NAN
    if func_type == "sin":
        return np.sin(x)
    if func_type == "mod":
...
def random_edge_mapping_with_random_lag(
    parents_data: List[np.ndarray],
    max_lag: int = 5
) -> np.ndarray:
    """Combine parent time-series with randomly chosen time-lag per parent, then apply activation."""
    L = parents_data[0].shape[0]
    lagged = []

    # 每个父节点单独采样一个 lag ∈ [1, max_lag]
    for p in parents_data:
        # 保留原始时序
        lagged.append(p)
        # 添加滞后时序
        lag = np.random.randint(1, max_lag + 1)  # 注意包含 max_lag
        shifted = np.roll(p, lag)
        shifted[:lag] = 0.0  # padding，前 lag 位设为 0
        lagged.append(shifted)

    combined = np.stack(lagged, axis=1)  # shape (L, K)
    ...
    # 随机激活
    chosen_func = np.random.choice(["linear", "relu", "sigmoid", "sin", "mod", "leakyrelu"])
    return random_activation(z, chosen_func)
# -----------------------------------------------------------------------------
...
def generate_scm_time_series_with_lag(
    *,
    ...
    """Generate one *network* of inter-linked GP-based time-series with per-parent random lags."""
    ...
    # ---- Sample roots directly from GP prior ----
    for r in root_nodes:
        selected_kernels = np.random.choice(
            kernel_bank, np.random.randint(1, 8), replace=True
        )
    ...
    # ---- Propagate through DAG with random lags ----
    for node in nx.topological_sort(dag):
        if node in root_nodes:
            continue
        parents = list(dag.predecessors(node))
        parents_ts = [node_data[p] for p in parents]
        node_data[node] = random_edge_mapping_with_random_lag(parents_ts, max_lag=max_lag)
...
def sample_time_series_from_scm_withTimeLag(
    *,
    time_length: int = 512,
    max_parents: int = 3,
    num_features: int = 6,
    ...
    node_data = generate_scm_time_series_with_lag(
        time_length=time_length,
        max_parents=max_parents,
        num_nodes=num_nodes,
        seed=seed,
...
def generate_multiple_scm_time_series(
    *,
    ...
    results: List[Dict] = []
    base_seed = seed
    for i in range(num_series // num_features):  # if num_features unchanged, nun_series be larger, it means more SCM graphs
        current_seed = base_seed + i * 100
        batch = sample_time_series_from_scm_withTimeLag(
            time_length=time_length,
            num_features=num_features,
            max_parents=max_parents,
            num_nodes=num_nodes,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", type=str, default="series", choices=["series", "episodes"], help="Output mode")

    # shared
    parser.add_argument("-N", "--num-series", type=int, default=1000000, help="total number of series to generate")
    parser.add_argument("-L", "--time-length", type=int, default=512, help="length of each individual series")
    parser.add_argument("-O", "--output-file", type=str, default="CauKerTimeLag1M_F5_P5M18.arrow", help="output file name")
    parser.add_argument("-F", "--features", type=int, default=5, help="how many nodes to sample per SCM draw")
    parser.add_argument("-P", "--max-parents", type=int, default=5, help="maximum #parents per node in the DAG")
    parser.add_argument("-M", "--num-nodes", type=int, default=18, help="total nodes in the DAG")
    parser.add_argument("--max-lag", type=int, default=5, help="maximum time lag for parent time series")

    # episodes mode
    parser.add_argument("--num-episodes", type=int, default=10000, help="number of episodes (mode=episodes)")
    parser.add_argument("--episodes-out-dir", type=str, default="./episodes_npz", help="output directory for episode npz files")
    parser.add_argument("--K", type=int, default=5, help="number of classes per episode")
    parser.add_argument("--n-ctx-per-class", type=int, default=2, help="context samples per class")
    parser.add_argument("--n-qry-per-class", type=int, default=5, help="query samples per class")
    parser.add_argument("--level", type=int, default=0, help="difficulty level")
    ...
    if args.mode == "series":
        ...
        from orion_msp.prior.synth_cauker_icl import generate_episode, generate_episodes_to_npz

        generate_episodes_to_npz(
            out_dir=args.episodes_out_dir,
            num_episodes=args.num_episodes,
            base_seed=args.seed,
            K=args.K,
            n_ctx_per_class=args.n_ctx_per_class,
            n_qry_per_class=args.n_qry_per_class,
            time_length=args.time_length,
            num_features=args.features,
            level=args.level,
            num_nodes=args.num_nodes,
        ...
        ctx_classes = set(np.unique(ep["y_ctx"]).tolist())
        qry_classes = set(np.unique(ep["y_qry"]).tolist())
        print(
            "[Episode Sanity] "
            f"x_ctx={ep['x_ctx'].shape}, y_ctx={ep['y_ctx'].shape}, "
            f"x_qry={ep['x_qry'].shape}, y_qry={ep['y_qry'].shape}, "
            f"qry_subset_ctx={qry_classes.issubset(ctx_classes)}"
        )
        print(f"Episode npz files saved to {Path(args.episodes_out_dir).resolve()}")
