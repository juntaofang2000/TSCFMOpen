...
class Trainer:
    ...
    def configure_ddp(self):
        ...
        if self.ddp:
            ...
            os.environ.setdefault("ORION_CUPY_DEVICE", str(self.ddp_local_rank))
        ...
        os.environ.setdefault("ORION_CUPY_IN_WORKER", "1")
    ...
    def build_model(self):
        model_name = getattr(self.config, "model", "orion_msp")

        if model_name == "rowmixer_lite_icl":
            # RowMixerLiteICL matches the same forward API used by the trainer:
            ...
            model = RowMixerLiteICL(**self.model_config).to(self.config.device)

            # Optional freezing flags reuse existing config toggles.
            if self.config.freeze_row:
                model.row_mixer.eval()
                for p in model.row_mixer.parameters():
        ...
        if self.ddp:
            find_unused = (model_name != "rowmixer_lite_icl") and (self.config.row_group_mode != "pma")
            self.model = DDP(
                model, 
    ...
    def train(self):
        ...
        for step in prog:
            ...
            if self.master_process:
                ...
                if bool(getattr(self.config, "log_prior_batch_time", False)):
                    log_every = max(1, int(getattr(self.config, "log_prior_batch_time_every", 1)))
                    if self.curr_step % log_every == 0:
                        msg = (
                            f"[rank0] step={self.curr_step} prior_time={prior_timer.elapsed:.3f}s "
                            f"train_time={train_timer.elapsed:.3f}s"
                        )
                        if isinstance(prog, tqdm):
                            prog.write(msg)
                        else:
