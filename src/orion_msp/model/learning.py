...
class ICLearning(nn.Module):
    ...
    def _icl_predictions(self, R: Tensor, y_train: Tensor) -> Tensor:
        ...
        Ey = self.y_encoder(y_train_long)
        R[:, :train_size] = R[:, :train_size] + Ey
    ...
    def _predict_hierarchical(self, R_test: torch.Tensor, softmax_temperature: float = 0.9) -> torch.Tensor:
        ...
        def process_node(node, R_test):
            ...
            if node.is_leaf:
                ...
                if leaf_preds.device != device:
                    leaf_preds = leaf_preds.to(device)
                global_preds = torch.zeros((test_size, num_classes), device=device)
                # Use Python ints to avoid device mismatch in indexing
                for local_idx, global_idx in enumerate(node.classes_.tolist()):
                    global_preds[:, int(global_idx)] = leaf_preds[:, local_idx]
                return global_preds
