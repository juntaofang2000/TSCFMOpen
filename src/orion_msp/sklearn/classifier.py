...
from ..model.inference_config import InferenceConfig, MgrConfig
...
class RowMixerLiteICLClassifier(ClassifierMixin, BaseEstimator):
    """RowMixerLiteICL 的 scikit-learn 分类器封装（接口与 OrionMSPClassifier 对齐）。

    整体流程复用 OrionMSPClassifier 的逻辑：
    - LabelEncoder 编码标签
    - TransformToNumerical 将输入转为数值
    - EnsembleGenerator 生成不同归一化/特征置换/类别 shift 的集成视图
    - 使用 RowMixerLiteICL 进行 in-context learning 推理

    说明：
    - 该类默认不支持 HuggingFace 自动下载（需要你显式提供 model_path）
    - use_hierarchical 的语义与 OrionMSPClassifier 一致（由 ICLearning 内部实现）
    """
    ...
    def _load_model(self):
        """从本地 checkpoint 加载 RowMixerLiteICL。

        期望 checkpoint 至少包含：
        - config: dict，可直接用于 RowMixerLiteICL(**config)
        - state_dict: 权重
        """

        if self.model_path is None:
            raise ValueError(
                "RowMixerLiteICLClassifier 需要显式提供 model_path（当前未配置 HuggingFace 自动下载）。"
            )
        ...
        try:
            ...
            # 兼容老版本 torch（无 weights_only 参数）
            checkpoint = torch.load(model_path_, map_location="cpu")
    ...
    def fit(self, X, y):
        ...
        # Load the pre-trained RowMixerLiteICL model if not already loaded or if model_path changed
        ...
        # Inference manager config (used by ICLearning)
        base_mgr = MgrConfig(device=self.device_, use_amp=bool(self.use_amp), verbose=bool(self.verbose))
        if self.mgr_config is None:
            self.mgr_config_ = base_mgr
        elif isinstance(self.mgr_config, dict):
            base_mgr.update(self.mgr_config)
            self.mgr_config_ = base_mgr
        else:
            raise TypeError("mgr_config must be a dict or None")
    ...
    def _batch_forward(self, Xs, ys):
        ...
        for X_batch, y_batch in zip(Xs, ys):
            X_batch = torch.from_numpy(X_batch).float().to(self.device_)
            y_batch = torch.from_numpy(y_batch).long().to(self.device_)
    ...
    def predict_proba(self, X):
        ...
        if self.n_jobs is not None:
            ...
            if self.n_jobs > 0:
                if self.n_jobs > n_logical_cores:
                    warnings.warn(
                        f"RowMixerLiteICL got n_jobs={self.n_jobs} but there are only {n_logical_cores} logical cores available."
                        f" Only {n_logical_cores} threads will be used."
                    )
                n_threads = min(n_logical_cores, self.n_jobs)
            else:
        ...
        for _norm_method, (Xs, ys) in data.items():
            outputs.append(self._batch_forward(Xs, ys))
        outputs = np.concatenate(outputs, axis=0)
        ...
        if self.average_logits:
            avg = OrionMSPClassifier.softmax(avg, axis=-1, temperature=self.softmax_temperature)
